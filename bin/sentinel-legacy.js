 ...
 */
function computeHardeningFindings(signals, manifest, findings) {
  ...
  Object.entries(probingRules.probes).forEach(([probeKey, probe]) => {
    ...
    const probeSignals = hardeningSignals.filter(s => {
      // Logic to match signals to probes (in this case, all current signals are logging)
      return probeKey === 'logging' && (s.id.startsWith('DEP_') || s.id.includes('LOG') || s.id.includes('TRACE'));
    });

    const hasStrong = probeSignals.some(s => s.probe_type === 'strong');
    const hasWeak = probeSignals.some(s => s.probe_type === 'weak');
    const hasTrace = probeSignals.some(s => s.probe_type === 'traceability');

    let verdict = 'FAIL';
    if (hasStrong || (hasWeak && hasTrace)) {
      verdict = 'PASS';
    } else if (hasWeak) {
      verdict = 'WEAK PASS';
    }

    // Only penalize if the article is actually claimed in manifest
    const isClaimed = declaredFlags.some(f => f.includes('log') || f.includes('traceability'));
    
    if (isClaimed) {
      if (verdict === 'FAIL') {
        findings.push({
          article, rule_id: 'EUAI-HARDENING-001',
          description: `[Implementation] Hardening FAIL: No technical proof of Logging/Traceability found in code despite claims.`,
          deduction: 20, severity: 'high', source: 'implementation',
          fix_snippet: "Implement a professional logging library (winston, pino) and traceability IDs.",
          hardening_verdict: 'FAIL'
        });
      } else if (verdict === 'WEAK PASS') {
        findings.push({
          article, rule_id: 'EUAI-HARDENING-002',
          description: `[Implementation] Hardening WEAK PASS: Found only basic console.log. Lacks industrial traceability/logging.`,
          deduction: 8, severity: 'medium', source: 'implementation',
          fix_snippet: "Upgrade from console.log to a robust logging framework with correlation IDs.",
          hardening_verdict: 'WEAK PASS'
        });
      }
    }
  });
...
function computeTrustMetrics(findings, manifest) {
  ...
  if (required.length === 0) return { claim_score: 0, evidence_score: 0, confidence: 'N/A', finalScore: 0 };

  const modules = Array.isArray(manifest.modules) ? manifest.modules : [];
  const declaredFlags = Array.isArray(manifest.declared_flags) ? manifest.declared_flags : [];
  
  // 1. Calculate CLAIM SCORE (Coverage)
  // How many of the required articles does the project CLAIM to satisfy?
  let claimedCount = 0;
  ...
  required.forEach(art => {
    ...
    if (isClaimed) {
      claimedCount++;
      claimedArticles.add(art);
    }
  });

  const claim_score = Math.round((claimedCount / required.length) * 100);

  // 2. Calculate EVIDENCE SCORE (Substance)
  // How many of the REQUIRED articles are actually VERIFIED (passed all findings)?
  const verified = determineVerifiedArticles(findings, manifest);
  const verifiedRequired = required.filter(a => verified.includes(a));
  
  // Base Evidence Score = % of required articles that are verified
  const baseEvidenceScore = Math.round((verifiedRequired.length / required.length) * 100);

  // Apply Substance Deductions specifically to evidence
  let deductions = 0;
  for (const f of findings) {
    if (required.includes(f.article)) {
      deductions += (f.deduction || 0) / required.length;
    }
  }
  const evidence_score = Math.max(0, Math.min(100, Math.round(baseEvidenceScore - deductions)));

  // 3. LEGACY SCORE (Simple)
  const finalScore = evidence_score;

  return {
    claim_score: 100, // Dummy for legacy
...
async function runCheck(args) {
  ...
  // 1.5 Autodiscovery for Hardening (Phase 3)
  ...
  // 2. Offline Evidence Validation (already done above, but keeping the comment for context if it was meant to be moved)
  // const manifestDir = path.dirname(path.resolve(manifestPath)); // Already defined
  // const evidenceFindings = validateEvidence(manifest, manifestDir); // Already defined
  
  // 3. Implementation Hardening Audit (Phase 3)
  const hardeningFindings = [];
  computeHardeningFindings(signals, manifest, hardeningFindings);

  // Combine all findings
  const combinedFindings = [...evidenceFindings, ...engineViolations, ...hardeningFindings];
  ...
  const trust = computeTrustMetrics(combinedFindings, manifest);
  ...
  console.log(`${C.gray}Claim Score (Coverage): ${C.white}${trust.claim_score}/100${C.reset}`);
  console.log(`${C.gray}Evidence Score (Proof): ${score < 50 ? C.red : C.white}${score}/100${C.reset}`);
  
  const confColor = trust.confidence === 'HIGH' ? C.green : (trust.confidence === 'MEDIUM' ? C.yellow : C.red);
  console.log(`${C.gray}Trust Confidence:      ${confColor}${C.bold}${trust.confidence}${C.reset}`);
  ...
  if (score < threshold) {
    const docFindings = combinedFindings.filter(f => f.source === 'evidence');
    const impFindings = combinedFindings.filter(f => f.source === 'implementation');
...
async function runFix(args) {
  ...
  const actionableRuleIds = ['EUAI-MIN-001', 'EUAI-TRANS-001', 'EUAI-OVER-002', 'EUAI-LOG-003', 'EUAI-HARDENING-001', 'EUAI-HARDENING-002'];
  ...
  if (allFindings.some(f => f.rule_id === 'EUAI-HARDENING-001' || f.rule_id === 'EUAI-HARDENING-002')) {
  ...
  const oldTrust = computeTrustMetrics(oldEvidenceFindings, manifest);
  ...
  const newTrust = computeTrustMetrics(newEvidenceFindings, patchedManifest);
